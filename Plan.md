...
- [x] **P1-1** 장중 자동 새로고침 (ETF 목록 2분 간격 폴링)
  - 정규장(regular) 상태일 때만 자동 새로고침, 장외/휴장일 중지
  - onMount setInterval / onDestroy clear
- [x] **P1-2** 전체 포지션 요약 뷰 (총 평가손익, 총 수익률)
  - EtfDetailPanel 빈 상태에 포트폴리오 요약 표시
  - PositionSummary.svelte 독립 컴포넌트 (포지션 탭)
- [x] **P1-3** 가상 포지션 탭 추가 (📈 그리드 | 📊 포지션 | 🔔 알림)
  - 모든 포지션 테이블 + 총 투자금액/평가금액/손익/수익률 요약
  - 행 클릭 → selectTicker
- [x] **P1-4** 이모지 수정: 👈 → 👉 (우측 그리드에서 종목 선택 안내)
- [x] **P1-5** 컬럼 선택 기능 (⚙️ 토글: 거래대금/전일종가/등락액)
  - EtfListItem에 prev_close, change_amount, trading_value 필드 추가
  - 백엔드 naver.rs JSON API에서 prevVal/valueTr 추출
  - localStorage에 컬럼 표시 상태 저장
- [x] **P1-6** 포지션 입력 종목 자동 하이라이트
  - $positions의 ticker에 좌측 보더 표시 (var(--accent))

### P1 남은 항목 (미구현)

- [ ] 네이버 일봉 차트 API fallback (기간별 등락률)
  - 현재 Yahoo Finance에만 의존 → Yahoo 장애 시 기간별 등락률 미출력
  - 네이버 일봉 API가 막혀있어 HTML 스크래핑만 가능 — P2로 검토
- [ ] ETF 카테고리 필터 (주식/채권/원자재/혼합 등)
  - 600+ ETF를 검색으로만 찾기 어려움, 자산군별 탭/필터 필요
- [ ] 접근성 개선 (svelte-check 경고 해결)
...
- [ ] 레버리지/인버스 경고 배지 — 종목명 키워드 기반 위험 표시 (높음)
- [ ] 저유동성 경고 — 거래량/거래대금 기준 "거래 적음" 표시 (높음)
- [ ] 쉬운 보기 모드 — 큰 글씨, 넓은 행, 핵심 컬럼만 (높음)
- [ ] 색상 의미 보조 텍스트 — 상승/하락/보합 아이콘 병기 (중간)
- [ ] 평가손익 기준 정렬/필터 — 수익 중/손실 중 필터 (높음)
- [ ] 배당/월배당 ETF 표시 — 키워드 기반 배지 (중간)
- [ ] 손익 알림 — 수익률 기준 알림 (+5%, -3%) (중간)
- [ ] 선택 종목 메모 — 개인 메모 저장 (중간)
- [ ] 관심 종목 그룹 — 배당/관찰/보유/위험 그룹 (중간)
- [ ] 데이터 신선도 표시 — 마지막 갱신 시간 (중간)
- [ ] CSV 내보내기 개선 — 포지션 평가손익 포함 (낮음)
